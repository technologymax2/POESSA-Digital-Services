...
const Verify = () => {
  ...
  const executeSearch = useCallback(async (query) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/pensioners/search?query=${query}`);
      ...
      setPensioner(res.data[0]);
      if (res.data[0].alreadyVerified) {
        alert("This pensioner has already completed verification for the current renewal period.");
      }
    } catch (err) {
      console.error(err);
      setPensioner(null);
      alert("Pensioner not found or server error.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const loadRenewal = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/renewals/current`);
      const res = await response.json();
      setRenewal(res.data);
      setRenewalStatus(res.status || "ACTIVE");
    } catch (err) {
      console.error(err);
      setRenewal(null);
      setRenewalStatus("ACTIVE"); // Fallback to ACTIVE if endpoint doesn't exist yet
    } finally {
            ...
            <div className="grid md:grid-cols-2 gap-8 border-t border-gray-200 pt-6">
              <div>
                ...
                <div className="mt-6">
                  ...
                  <img src={`${API_BASE_URL}${pensioner.image}`} alt={pensioner.nameEng} className="w-64 h-64 object-cover rounded-lg border border-gray-200 shadow-sm" />
                </div>
              </div>
