...
function Report() {
  ...
  // 🔄 ሁሉንም የጡረተኞች የባዮሜትሪክስ መረጃ አንድ ላይ የሚያመጣ ፈንክሽን
  ...
  // 🟢 🔴 የማረጋገጫ ሁኔታን ማሻሻያ ፈንክሽን (Verified / Failed)
    ...
    <div className="admin-dashboard-wrapper">
      ...
      <div className="admin-summary-grid">
        ...
        <div className="summary-card rejected">
          <h3>{pensioners.filter(p => getStatus(p) === "Failed" || getStatus(p) === "Rejected").length}</h3> 
          <p>ውድቅ የተደረጉ</p>
        </div>
      </div>
            ...
            {loading ? (
              ...
              filteredData.map((p) => (
                <tr key={p._id || p.faydaNumber}>
                  ...
                  <td>{p.name || p.nameAmh || p.nameEng || "ስም አልተጠቀሰም"}</td>
                  <td>
                    <span style={{ 
                      padding: "6px 12px", 
                      borderRadius: "20px", 
                      fontSize: "12px", 
                      fontWeight: "bold", 
                      background: (getStatus(p) === "Verified") ? "#dcfce7" : (getStatus(p) === "Failed" || getStatus(p) === "Rejected") ? "#fee2e2" : "#fef9c3", 
                      color: (getStatus(p) === "Verified") ? "#16a34a" : (getStatus(p) === "Failed" || getStatus(p) === "Rejected") ? "#dc2626" : "#ca8a04" 
                    }}>
                      {getStatus(p)}
                    </span>
                  </td>
                  <td style={{ textAlign: "center" }} className="no-print">
                    <button onClick={() => { setSelectedPensioner(p); setComment(p.comment || ""); }} style={{ background: "#162447", color: "#fff", border: "none", padding: "6px 14px", borderRadius: "6px", cursor: "pointer", fontWeight: "bold", fontSize: "13px" }}>
                      🔍 መረጃውን መርምር
                    </button>
                  </td>
                </tr>
      ...
      {/* 🔍 የባዮሜትሪክስ ፎቶዎች ማነጻጸሪያ (Modal Pop-up) */}
      {selectedPensioner && (
        <div className="admin-modal-overlay">
          <div className="admin-modal-content">
            ...
            <p style={{ margin: "5px 0" }}><strong>የጡረተኛው ስም፦</strong> {selectedPensioner.name || selectedPensioner.nameAmh || selectedPensioner.nameEng || "ያልታወቀ"}</p>
            ...
            <div style={{ display: "flex", gap: "20px", margin: "20px 0", justifyContent: "center" }}>
              <div style={{ textAlign: "center" }}>
                ...
                {/* 💡 ማስተካከያ፦ ከ photoUrl ይልቅ በሞዴሉ መሠረት idPhoto ተደርጓል */}
                <img src={selectedPensioner.idPhoto || selectedPensioner.photoUrl || "https://via.placeholder.com/150"} alt="Database" style={{ width: "150px", height: "150px", objectFit: "cover", borderRadius: "8px", border: "2px solid #cbd5e1" }} />
              </div>
              <div style={{ textAlign: "center" }}>
            ...
            <div style={{ background: "#f8fafc", padding: "10px", borderRadius: "6px", marginBottom: "15px", fontSize: "13px" }}>
              ...
              <div>🔄 እንቅስቃሴ አልፏል? <strong>{selectedPensioner.nodPassed || selectedPensioner.turnPassed ? "✅ አዎ" : "❌ የለም"}</strong></div>
            </div>
