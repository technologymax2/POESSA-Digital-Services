...
import { useNavigate } from 'react-router-dom'; // ➕ ለዳሽቦርድ እና ሎጋውት ጉዞ
import { QRCodeSVG } from 'qrcode.react';
import './PensionerRegistration.css';

function PensionerRegistration() {
  const navigate = useNavigate(); // ➕ የኔቪጌሽን መቆጣጠሪያ
  const [currentEmployee, setCurrentEmployee] = useState('ባለሙያ'); // የባለሙያው ስም መያዣ
  ...
  // ➕ አዲስ፡ ወደ ዳሽቦርድ መመለሻ ተግባር
  const handleGoToDashboard = () => {
    navigate('/dashboard'); // ወይም ያንተ ዳሽቦርድ አድራሻ ወዳለበት (ለምሳሌ /employee-dashboard)
  };

  // ➕ አዲስ፡ ከሲስተም መውጫ ተግባር (Logout)
  const handleLogout = () => {
    if (window.confirm("እርግጠኛ ነዎት ከሲስተሙ መውጣት ይፈልጋሉ?")) {
      localStorage.clear(); // መረጃዎችን ማጽዳት
  ...
  // 📝 መረጃ ማሻሻል (Update) - የባለሙያውን ስም አብሮ ይልካል 👤
  const handleUpdate = async (e) => {
    ...
    try {
      const response = await fetch(`https://poessa-digital-services-1.onrender.com/api/pensioners/update/${registeredData._id}`, {
        ...
        body: JSON.stringify({ ...editData, employeeName: currentEmployee }), // የባለሙያ ስም ተጨምሯል
      });
  ...
  // 🗑️ መረጃን ማጥፋት (Delete) - ያጠፋውን ባለሙያ ስም ይልካል 👤
  ...
  // 📥 አዲስ መረጃ መመዝገብ (Register) - የመዘገበውን ባለሙያ ስም ይልካል 👤
  const handleSubmit = async (e) => {
    ...
    dataToSend.append('employeeName', currentEmployee); // የባለሙያ ስም ተጨምሯል
  ...
  return (
    <div className="registration-container">
      ...
      {/* ➕ አዲስ፡ የሰራተኛው መቆጣጠሪያ ቶፕ ባር (Top Navigation Bar) */}
      <div className="no-print" style={{ display: 'flex', justifyContent: 'between', alignItems: 'center', backgroundColor: '#2d3748', color: 'white', padding: '12px 20px', borderRadius: '6px', marginBottom: '25px' }}>
        <div style={{ flex: 1 }}>
